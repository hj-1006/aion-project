  ...
  async function loadUsers() {
    ...
    try {
      ...
      tbody.innerHTML = users.map(function (u) {
        ...
        return '<tr><td>' + (u.id || '-') + '</td><td>' + (u.username || '-') + '</td><td>' + (u.display_name || '-') + '</td><td>' + roleLabel(u.role) + '</td><td>' + (u.email || '-') + '</td><td>' + (useMail ? '예' : '아니오') + '</td><td><button type="button" class="btn-edit btn-icon-only" data-id="' + (u.id || '') + '" data-username="' + (u.username || '').replace(/"/g, '&quot;') + '" data-display_name="' + (u.display_name || '').replace(/"/g, '&quot;') + '" data-email="' + (u.email || '').replace(/"/g, '&quot;') + '" data-can_use_mail="' + (useMail ? '1' : '0') + '" data-role="' + (u.role || 'user') + '" aria-label="수정" title="수정"><svg class="icon" aria-hidden="true"><use href="/icons/sprite.svg#icon-pencil"/></svg></button></td></tr>';
      }).join('');
