...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!username || !password) {
      return res.status(400).json({ success: false, message: 'username과 password를 입력하세요.' });
    }
    ...
    req.session.save(async (err) => {
      ...
      res.json({ success: true, user: { username: user.username, display_name: user.display_name } });
    });
  } catch (err) {
    console.error(err);
